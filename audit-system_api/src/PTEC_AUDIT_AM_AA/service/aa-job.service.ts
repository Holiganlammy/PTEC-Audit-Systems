...
export class AAJobsService {
  ...
  async findAll(
    ...
    if (roleId === 1) {
      ...
      // Master Area: เห็นทุกสาขาของ AA ได้ไม่จำกัด (read-only ในทางปฏิบัติ
      // เพราะ mutation ต่างๆ ยังต้องผ่านการเช็คสิทธิ์แก้ไขที่หน้า edit ตาม ownership ของ job)
    } else if (roleId === 3) {
      // AM: เห็น job ที่ตัวเองเป็น amManager หรือเป็นคนสร้าง (ไม่ขยายไปทั้งสาขา
      ...
      // AA: เห็น job ที่ตัวเองเป็น aaUser หรือเป็นคนสร้าง (ไม่ขยายไปทั้งสาขา
      // เพราะคนตรวจของสาขาเปลี่ยนได้เรื่อยๆ — สาขาเดียวกันแต่คนตรวจคนละคนไม่ควรเห็นกัน)
  ...
  async update(
    ...
    if (updateDto.branchAssignment !== undefined)
      aaJob.branchAssignment = updateDto.branchAssignment;
