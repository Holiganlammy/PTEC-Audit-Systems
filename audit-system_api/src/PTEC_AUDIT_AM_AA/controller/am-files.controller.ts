...
@Controller('am-items')
export class AMFilesController implements OnModuleInit {
  constructor(private readonly AMFilesService: AMFilesService) {}
  ...
  async getAttachmentsByJob(
    ...
    success: boolean;
    data: Record<number, Array<AMFiles & { fileUrl: string }>>;
    totalFiles: number;
  }> {
    try {
      const result = await this.AMFilesService.getAttachmentsByJobId(jobId);
      return {
        success: true,
        data: result.grouped,
  ...
  ): Promise<{ success: boolean; data: AMFiles[]; message: string }> {
    try {
      ...
      const uploadedFiles = await Promise.all(
        files.map((file) =>
          this.AMFilesService.uploadItemAttachment({
            itemId,
  ...
  async getItemAttachments(@Param('id', ParseIntPipe) itemId: number): Promise<{
    success: boolean;
    data: Array<AMFiles & { fileUrl: string }>;
  }> {
    try {
      const files = await this.AMFilesService.getItemAttachments(itemId);
  ...
  async downloadItemAttachment(
    ...
    try {
      const file = await this.AMFilesService.getItemAttachmentFile(
        itemId,
        fileId,
      );
  ...
  async deleteItemAttachment(
    ...
    try {
      await this.AMFilesService.deleteItemAttachment(itemId, fileId, deletedBy);
  ...
  async deleteAMChecklistAttachment(
    ...
    try {
      ...
      await this.AMFilesService.deleteFile(fileId, userId);
