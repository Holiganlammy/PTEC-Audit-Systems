...
export class AuditJobsController implements OnModuleInit {
  constructor(
    private readonly amJobsService: AMJobsService,
  ...
  async create(
    ...
    try {
      const auditJob = await this.amJobsService.create(createAuditJobDto);
  ...
  async findAll(
    ...
    return await this.amJobsService.findAll(params, user);
  }
  ...
  async findOneByJobCode(
    ...
    try {
      ...
      const auditJob = await this.amJobsService.findByJobNo(normalizedJobCode);
  ...
  async findOneByJobCodeQuery(
    ...
    try {
      ...
      const auditJob = await this.amJobsService.findByJobNo(normalizedJobNo);
  ...
  async findByStatus(
    ...
    try {
      const auditJobs = await this.amJobsService.findByStatus(status);
      return res.status(HttpStatus.OK).json({
        success: true,
        data: auditJobs,
      });
  ...
  async findOne(
    ...
    try {
      ...
      const auditJob = await this.amJobsService.findOne(id);
  ...
  async update(
    ...
    try {
      const auditJob = await this.amJobsService.update(id, updateAuditJobDto);
      return res.status(HttpStatus.OK).json({
        success: true,
        data: auditJob,
  ...
  async updateStatus(
    ...
    try {
      const amJob = await this.amJobsService.updateStatus(
        id,
        status,
        updatedBy || 0,
      );
      return res.status(HttpStatus.OK).json({
        success: true,
        data: amJob,
        message: `AM job status updated to ${status} successfully`,
      });
    } catch (error) {
      console.error('Error updating AM job status:', error);
      if (error instanceof NotFoundException) {
        return res.status(HttpStatus.NOT_FOUND).json({
          success: false,
          message: error.message,
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
        success: false,
        message: 'Error updating AM job status',
      });
    }
  }
  ...
  async confirm(
    ...
    try {
      const auditJob = await this.amJobsService.confirm(id, confirmedBy || 0);
      return res.status(HttpStatus.OK).json({
        success: true,
        data: auditJob,
  ...
  async remove(
    ...
    try {
      await this.amJobsService.remove(id, body.delete_reason, body.deleted_by);
      return res.status(HttpStatus.OK).json({
        success: true,
  ...
  async hardDelete(
    ...
    try {
      await this.amJobsService.delete(id);
      return res.status(HttpStatus.OK).json({
        success: true,
