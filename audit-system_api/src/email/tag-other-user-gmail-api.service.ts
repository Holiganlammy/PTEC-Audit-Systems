...
<body style="margin: 0; padding: 0; background-color: #f5f5f5; font-family: 'Sarabun', Arial, sans-serif;">
  ...
  async sendTaggedUserReplyEmail(params: {
    to: string;
    ...
    const taggerName = params.taggerFullname?.trim() || 'ผู้ใช้งาน';
    const replierName = params.replierFullname?.trim() || 'หน่วยงานที่เกี่ยวข้อง';
    const itemName = params.itemName?.trim() || 'รายการตรวจสอบ';
    const jobNo = params.jobNo?.trim() || '-';
    const branchName = params.branchName?.trim() || '-';
    const formType = params.formType?.toUpperCase() || 'Audit';
    ...
    const subject = `${replierName} มีการตอบกลับคอมเมนต์ JobNo #${params.jobNo}`;
    const html = `
<!DOCTYPE html>
<html>
<head>
    ...
    <tr>
      <td align="center" style="padding: 40px 0;">
        <table role="presentation" style="width: 600px; max-width: 90%; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #1E40AF 0%, #3B82F6 100%); padding: 24px 32px; text-align: center;">
              <h1 style="margin: 0; color: #000000; font-size: 22px; font-weight: bold;">แจ้งเตือนการตอบกลับคอมเมนต์</h1>
              <p style="margin: 6px 0 0; color: #000000; font-size: 13px;">${formTypeLabel} — PTEC Audit System</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <!-- Greeting -->
              <p style="margin: 0 0 24px; font-size: 16px; color: #1F2937;">
                เรียน <strong>${taggerName}</strong>
              </p>

              <!-- Message -->
              <p style="margin: 0 0 24px; font-size: 15px; line-height: 1.6; color: #374151;">
                <strong>${replierName}</strong> ซึ่งเป็นหน่วยงานที่เกี่ยวข้องที่คุณแท็กไว้ ได้ตอบกลับคอมเมนต์ในรายการตรวจสอบที่คุณแสดงความคิดเห็นไว้
                ...
                <tr>
                  <td style="padding: 16px; background-color: #F9FAFB;">
                    <table role="presentation" style="width: 100%;">
                      ...
                      <tr>
                        <td style="padding: 8px 0; font-size: 14px; color: #6B7280;">ข้อความตอบกลับ:</td>
                        <td style="padding: 8px 0; font-size: 14px; color: #1F2937;">${params.commentText}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
