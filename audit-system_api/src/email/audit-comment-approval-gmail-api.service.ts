...
<body style="margin: 0; padding: 0; background-color: #f5f5f5; font-family: 'Sarabun', Arial, sans-serif;">
  ...
  /**
   * ส่งเมลแจ้งผลอนุมัติ/ไม่อนุมัติกลับไปหาผู้ร้องขอ (เจ้าของ Comment)
   * @param params - ข้อมูลสำหรับส่งเมล
   */
  async sendCommentApprovalResultEmail(params: {
    requesterEmail: string;
    requesterFullname: string;
    approverFullname: string;
    approverPosition?: string;
    resultStatus: number; // 1 = อนุมัติ, 2 = ไม่อนุมัติ
    commentText: string;
    approverNote?: string;
    jobNo: string;
    categoryName: string;
    ...
    const themeColor = isApproved ? '#16A34A' : '#6B7280';
    const themeColorLight = isApproved ? '#DCFCE7' : '#F3F4F6';
    const statusText = isApproved ? 'อนุมัติแล้ว' : 'ไม่อนุมัติ';
    const subject = `${statusText} Comment [${formTypeLabel}]: ${jobNo}`;
...
<body style="margin: 0; padding: 0; background-color: #f5f5f5; font-family: 'Sarabun', Arial, sans-serif;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
        ...
        <table role="presentation" style="width: 600px; max-width: 90%; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="background: ${themeColor}; padding: 24px 32px; text-align: center;">
              <h1 style="margin: 0; color: #ffffff; font-size: 22px; font-weight: bold;">${statusText} Comment</h1>
              <p style="margin: 6px 0 0; color: #ffffff; font-size: 13px;">${formTypeLabel} — PTEC Audit System</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <!-- Greeting -->
              <p style="margin: 0 0 24px; font-size: 16px; color: #1F2937;">
                เรียน <strong>${requesterName}</strong>
              </p>

              <!-- Message -->
              <p style="margin: 0 0 24px; font-size: 15px; line-height: 1.6; color: #374151;">
                Comment ที่ท่านส่งขออนุมัติ ได้รับการ<strong>${statusText}</strong>จาก<br/>
                <strong>${approverName}</strong>${approverPosition ? ` (${approverPosition})` : ''} แล้ว
              </p>

              <!-- Comment Box -->
              <div style="background-color: #F9FAFB; border-left: 4px solid ${themeColor}; padding: 16px; margin-bottom: 16px; border-radius: 4px;">
                <p style="margin: 0; font-size: 14px; color: #6B7280; font-weight: 600; margin-bottom: 8px;">Comment ของท่าน:</p>
                <p style="margin: 0; font-size: 14px; color: #1F2937; line-height: 1.5;">"${commentText}"</p>
              </div>

              ${
                approverNote
                  ? `<div style="background-color: ${themeColorLight}; padding: 16px; margin-bottom: 24px; border-radius: 4px;">
                <p style="margin: 0; font-size: 14px; color: #6B7280; font-weight: 600; margin-bottom: 8px;">หมายเหตุจากผู้อนุมัติ:</p>
                <p style="margin: 0; font-size: 14px; color: #1F2937; line-height: 1.5;">"${approverNote}"</p>
              </div>`
                  : ''
              }
                ...
                <tr>
                  <td style="padding: 16px; background-color: #F9FAFB;">
                    <table role="presentation" style="width: 100%;">
                      ...
                      <tr>
                        <td style="padding: 8px 0; font-size: 14px; color: #6B7280;">${statusText}โดย:</td>
                        <td style="padding: 8px 0; font-size: 14px; color: #1F2937;">${approverName}${approverPosition ? ` (${approverPosition})` : ''}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
              ...
              <table role="presentation" style="width: 100%; margin-bottom: 24px;">
                <tr>
                  <td align="center">
                    <a href="${commentUrl}"
                       style="display: inline-block; padding: 12px 32px; background-color: ${themeColor}; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 15px;">
                      ดูรายละเอียด
                    </a>
                  </td>
                </tr>
    ...
    try {
      ...
      console.log(`✓ Approval result email sent to ${params.requesterEmail}`);
    } catch (error) {
      console.error(
        `✗ Failed to send approval result email to ${params.requesterEmail}:`,
        error,
      );
      throw error;
    }
