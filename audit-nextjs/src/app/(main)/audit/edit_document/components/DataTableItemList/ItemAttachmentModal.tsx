    ...
    const loadPreview = async () => {
      ...
      try {
        const response = await client.get(
          `/audit-items/${itemId}/attachments/${file.fileId}/view`,
          {
...
export default function ItemAttachmentModal({
  ...
  useEffect(() => {
    ...
    const fetchFiles = async () => {
      setIsLoading(true);
      try {
        const response = await client.get(`/audit-items/${item.item_id}/attachments`, {
          headers: dataConfig().headers,
        });
        ...
        toast.error("ไม่สามารถโหลดไฟล์แนบของรายการได้", {
          description: getErrorMessage(error, "ไม่สามารถโหลดไฟล์แนบของรายการได้"),
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await client.post(
        `/audit-items/${item.item_id}/attachments`,
        formData,
        {
          headers: {
            ...dataConfig().headers,
      ...
      if (uploadedFiles.length > 0) {
        ...
        const filesResponse = await client.get(`/audit-items/${item.item_id}/attachments`, {
          headers: dataConfig().headers,
        });
        setExistingFiles(
      ...
      toast.error("ไม่สามารถอัปโหลดไฟล์ได้", {
        description: getErrorMessage(error, "ไม่สามารถอัปโหลดไฟล์ได้"),
      });
    } finally {
  ...
  const handleDownloadFile = async (fileId: number, fileName: string) => {
    ...
    try {
      const response = await client.get(
        `/audit-items/${item.item_id}/attachments/${fileId}/download`,
        {
          headers: dataConfig().headers,
          responseType: "blob",
        }
  ...
  const handleDeleteExistingFile = async (fileId: number) => {
    ...
    try {
      await client.delete(`/audit-items/${item.item_id}/attachments/${fileId}`, {
        headers: dataConfig().headers,
