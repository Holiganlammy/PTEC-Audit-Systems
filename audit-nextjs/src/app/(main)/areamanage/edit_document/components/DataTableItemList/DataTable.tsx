...
export default function DataTableItemList({
  ...
  useEffect(() => {
    client.get("/am-items/all/tagged-users", { headers: dataConfig().headers })
      .then((res) => {
  ...
  const fetchAttachmentCounts = useCallback(async () => {
    ...
    try {
      const res = await client.get(`/am-items/attachments/by-job/${jobId}`, {
        headers: dataConfig().headers,
      });
  ...
  const { visibleItems, accessDenied } = useMemo(() => {
    ...
    // AM: roles 1,2,3,4,9,10 see all items; AA: roles 1,2,3,4,8,9,10 see all items
    const fullAccessRoles = positionType === "AA" ? [1, 2, 3, 4, 8, 9, 10] : [1, 2, 3, 4, 9, 10];
  ...
  const confirmDelete = async () => {
    ...
    try {
      await client.delete(`/am-items/${deleteItem.item_id}`, { headers: dataConfig().headers });
      toast.success("ลบรายการสำเร็จ"); onItemsChange();
    } catch (error) { toast.error("ไม่สามารถลบรายการได้", { description: getErrorMessage(error, "กรุณาลองใหม่อีกครั้ง") }); }
