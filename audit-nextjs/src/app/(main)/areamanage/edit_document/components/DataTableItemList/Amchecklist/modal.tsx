...
function FilePreviewModal({
  ...
  useEffect(() => {
    ...
    client.get(`/am-items/${itemId}/am-checklist/attachments/${file.fileId}/view`, {
...
export default function AMChecklistModal({
  ...
  useEffect(() => {
    if (open && item) {
      ...
      Promise.all([
        client.get(`/am-items/${item.item_id}/am-checklist`, {
          headers: dataConfig().headers,
        }),
        client.get(`/am-items/${item.item_id}/am-checklist/attachments`, {
          headers: dataConfig().headers,
        }),
      ])
  ...
  const handleDownloadFile = async (fileId: number, fileName: string) => {
    ...
    try {
      const response = await client.get(
        `/am-items/${item.item_id}/am-checklist/attachments/${fileId}/download`,
        {
          headers: dataConfig().headers,
          responseType: 'blob',
        }
  ...
  const handleDeleteExistingFile = async (fileId: number) => {
    ...
    try {
      await client.delete(
        `/am-items/${item.item_id}/am-checklist/attachments/${fileId}`,
        { headers: dataConfig().headers }
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      await client.patch(
        `/am-items/${item.item_id}/am-checklist`,
        {
      ...
      if (files.length > 0) {
        ...
        await client.post(
          `/am-items/${item.item_id}/am-checklist/attachments`,
          formData,
          {
            headers: {
              ...dataConfig().headers,
      ...
      toast.error("ไม่สามารถบันทึกได้", { description: getErrorMessage(error, "กรุณาลองใหม่อีกครั้ง") });
    } finally {
  ...
  const handleClear = async () => {
    ...
    setIsSubmitting(true);
    try {
      await client.delete(`/am-items/${item.item_id}/am-checklist`, {
        headers: dataConfig().headers,
      });
      ...
      toast.error("ไม่สามารถล้างข้อมูลได้", { description: getErrorMessage(error, "กรุณาลองใหม่อีกครั้ง") });
    } finally {
      setIsSubmitting(false);
    }
  };
