...
export default function NoteCell({
  ...
  // Sync comments from parent when modal is closed (draft mode or after items reload)
  useEffect(() => {
    if (!openThread || itemId <= 0) {
      setComments(initialComments);
    }
  }, [initialComments, openThread, itemId]);
  ...
  useEffect(() => {
    const fetchComments = async () => {
      ...
      try {
        ...
        toast.error("ไม่สามารถโหลดความเห็นได้", { description: getErrorMessage(error, "กรุณาลองใหม่อีกครั้ง") });
      } finally {
        setIsLoading(false);
      }
    };
...
const handleSubmit = async (text: string, approverStatus: 0 | null = null) => {
  ...
  const response = await client.get(`/am-items/${itemId}/${endpoint}`, {
    headers: dataConfig().headers,
  });
  ...
  if (response.data.success) {
    ...
    if (approverStatus === 0) {
      ...
      try {
        const taggedRes = await client.get(`/am-items/${itemId}/tagged-user`, {
          headers: dataConfig().headers,
        });
  ...
  const handleDelete = async (commentId: number, remark: string) => {
    ...
    await client.delete(`/am-items/${itemIdParam}/${endpoint}/${commentIdParam}`,{
      headers: dataConfig().headers,
      data: { updatedBy: session?.user?.UserID, deletedReason: remarkText },
    });

    // Refetch comments
    const response = await client.get(`/am-items/${itemIdParam}/${endpoint}`, {
      headers: dataConfig().headers,
    });
  ...
  const handleEdit = async (commentId: number, note: string) => {
    ...
    await client.put(`/am-items/${itemIdParam}/${endpoint}/${commentIdParam}`, payload, {
      headers: dataConfig().headers,
    });
  ...
  const handleApprove = async (commentId: number, status: 1 | 2) => {
    ...
    await client.patch(
      `/am-items/${itemId}/${endpoint}/${commentId}/approve`,
      payload,
      {
        headers: dataConfig().headers,
      }
