...
export default function EditAuditJobPage() {
  ...
  const fetchJobData = useCallback(async () => {
    try {
      ...
      setRoleFormTab(jobData.positionType === "AA" ? "AA" : "AM");
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const items = response.data.data.map((item: AuditItemData) => ({
            ...
            headerChecklistStatus: item.headerChecklistStatus ?? null,
            headerChecklistDetail: item.headerChecklistDetail,
            headerChecklistBy: item.headerChecklistBy,
            headerChecklistAt: item.headerChecklistAt,
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      try {
        const { doc, filename } = await buildAuditDetailPdf(jobData, auditItems, roleFormTab);
        ...
        branchFormData.append("formType", roleFormTab);

        const branchResponse = await client.post(
          "/am-email/send-branch-summary",
          branchFormData,
  ...
  const handleResetStatus = async () => {
    ...
    setIsResetting(true);
    try {
      const endpoint = roleFormTab === "AA"
        ? `/aa-jobs/${jobData.jobId}/status`
        : `/am-jobs/${jobData.jobId}/status`;
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                <h1 className="text-xl font-bold sm:text-2xl lg:text-3xl">Edit Area Manager Job</h1>
                    ...
                    <span tabIndex={jobData?.jobCreatedEmailSentAt || !canSendSummaryEmail ? 0 : undefined}>
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button
                            variant="outline"
                            disabled={!canEdit || isLoadingData || isSendingEmail || !jobData || !!jobData?.jobCreatedEmailSentAt || !canSendSummaryEmail}
                    ...
                    <TabsContent value="AM" className="mt-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Controller
                          ...
                          render={({ field, fieldState }) => (
                            <Field>
                              <FieldLabel>RM <span className="text-red-500">*</span></FieldLabel>
